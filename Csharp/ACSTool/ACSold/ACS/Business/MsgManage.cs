...
namespace ACS
{
    public class MsgManage
    {
        ...
        public static void DataTranslate(SocketOpt socketOpt)
        {
            ...
            agvMsg.Barcode = Encoding.ASCII.GetString(DataRecv, 13,4);
            ...
            string sid = Encoding.ASCII.GetString(DataRecv, 26, 4).Replace("\0", "");
            if (string.IsNullOrEmpty(sid))
                agvMsg.SID = 0;
            else
                agvMsg.SID = int.Parse(sid);
            agvMsg.sTaskType = (STaskType)DataRecv[30];
