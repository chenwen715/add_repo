...
namespace statistics
{
    ...
    public class DAL_Comn_Excel
    {
        ...
        //获取一个工作表
        {
            Microsoft.Office.Interop.Excel.Worksheet s = null;
        ...
        {
            Microsoft.Office.Interop.Excel.Sheets allsheets = wb.Worksheets;
            foreach (Microsoft.Office.Interop.Excel.Worksheet wss in allsheets)
            {
                string name = wss.Name;
                if (name == sheetName)
                {
                    return true;
                }
        ...
        public void SetCellColor(Microsoft.Office.Interop.Excel.Worksheet ws, int x, int start,int columnCount,Color color)
        //ws：要设值的工作表     X行Y列     value   值
        {
            Microsoft.Office.Interop.Excel.Range titleRange = ws.Range[ws.Cells[x, start], ws.Cells[x, start+columnCount-1]];
            titleRange.Interior.Color = color;
        }
